        ...
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
            ...
            <div class="mb-3">
                ...
                <select name="class" id="class" class="form-control @error('class') is-invalid @enderror" required>
                    ...
                    <option value="text-bg-secondary" {{ (old("class",$category->class) == "text-bg-secondary" ? "selected":"") }}>Grigio</option>
                    <option value="text-bg-success" {{ (old("class",$category->class) == "text-bg-success" ? "selected":"") }}>Verde</option>
                    <option value="text-bg-danger" {{ (old("class",$category->class) == "text-bg-danger" ? "selected":"") }}>Rosso</option>
                    <option value="text-bg-warning" {{ (old("class",$category->class) == "text-bg-warning" ? "selected":"") }}>Giallo</option>
                    <option value="text-bg-info" {{ (old("class",$category->class) == "text-bg-info" ? "selected":"") }}>Azzurro</option>
                    <option value="text-bg-light" {{ (old("class",$category->class) == "text-bg-light" ? "selected":"") }}>Bianco</option>
                    <option value="text-bg-dark" {{ (old("class",$category->class) == "text-bg-dark" ? "selected":"") }}>Nero</option>
                </select>
                    @error('class')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
